...
namespace smart {
    ...
    */
    ...
    // LineBreakNodeStruct is specifically used for representing line breaks in the code, it will be linked to the code node generated by tokenizer, and it will be used for generating code later, so it needs to be a separate node from the code node, and it will be linked to the code node through prevLineBreakNode, so that we can easily generate line breaks in the generated code.
