...
namespace smart {
    ...
    */
    ...
    #define NODE_HEADER \
        ...
        bool typeAtHeap;                \
        int foundPos; \
        /* prevSpaceCount holds number of chars before this node, used for error reporting and code generation. */ \
    ...
    // LineBreakNodeStruct is used for storing line break info for error reporting and code generation. It is linked to the code node generated by tokenizer, and it allows to keep line break info even if there are multiple line breaks before the next code node.
    ...
    */
    ...
    struct ParseContext {
        ...
        NodeBase *mostLeftNode; // store the most left node generated by tokenizer, used for assigning line break and comment nodes in scanWithTokenizer
        NodeBase *valueNode;
        NodeBase *generatedMainNode;
        // set the most left node of the first tokenized node in this scan,
        // which will be used for assigning line break and comment nodes in the next scan
        NodeBase *firstTokenizedLeftNode;
